...
{if isset($groups)}
	...
	{foreach from=$combinations key=idCombination item=combination}
		...
		addCombination({$idCombination|intval}, new Array({$combination.list}), {$combination.quantity}, {$combination.price}, {$combination.ecotax}, {$combination.id_image}, '{$combination.reference|addslashes}', {$combination.unit_impact}, {$combination.minimal_quantity}, {$combination.maximum_quantity}, available_date, specific_price_combination);
	{/foreach}
{/if}
	...
	<div id="pb-left-column">
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html'}" method="post">
			...
			<p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
				...
				<!-- maximum quantity wanted -->
				<p id="maximum_quantity_wanted_p"{if $product->maximum_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
					{l s='This product is not sold individually. You must select at least'} <b id="maximum_quantity_label">{$product->maximum_quantity}</b> {l s='quantity for this product.'}
				</p>
				{if $product->maximum_quantity > 1}
					<script type="text/javascript">
			...
			{else}
				{foreach $combination as $key => $value}
					{if $key == "minimal_quantity"}
						{$minimal_quantity_combination = $value}
					{/if}
					{if $key == "minimal_quantity"}
				...
				<!-- maximum quantity wanted -->
				<p id="maximum_quantity_wanted_p"{if $product->maximum_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
					{l s='This combination is not sold individually. You must select at least'} <b id="maximum_quantity_label">{$maximum_quantity_combination}</b> {l s='quantity for this product.'}
				</p>
				{if $product->maximum_quantity > 1}
					<script type="text/javascript">
						checkMaximumQuantity();
