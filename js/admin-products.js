...
product_tabs['Combinations'] = new function(){
		...
		function editProductAttribute (url, parent){
			$.ajax({
				...
				success: function(data) {
					...
					var maximum_quantity = data[0]['maximum_quantity'];
	...
	ean, quantity, image, old_attr, id_product_attribute, default_attribute, eco_tax, upc, minimal_quantity, maximum_quantity, available_date)
	{
		var link = '';
		self.init_elems();
		...
		$('#attribute_maximum_quantity').val(maximum_quantity);
...
var ProductMultishop = new function()
{
	...
	this.checkAllCombinations = function()
	{
		...
		ProductMultishop.checkField($('input[name=\'multishop_check[attribute_maximum_quantity]\']').prop('checked'), 'attribute_maximum_quantity');
